...
AUTH_RESULT=`curl -sS -k -H "X-XSRF-TOKEN: $XSRF_TOKEN" -H 'Content-Type: application/json' -b $COOKIE -c $COOKIE -X POST \
...
if [[ "$AUTH_RESULT" =~ error ]] ; then
	...
	curl -sS -k -H "X-XSRF-TOKEN: $XSRF_TOKEN" -b $COOKIE -X POST \
	...
	if [ $ID -gt 0 ] ; then
		...
		RESULT=`curl -sS -k -H "X-XSRF-TOKEN: $XSRF_TOKEN" -H 'Content-Type: application/json' -b $COOKIE -X POST \
...
# clear temp files
rm -f $TMP_DIR/ksmganswer.json $CERT_FILE $COOKIE
